...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 14, 2021 at 10:22 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERT_OR_SET` (IN `P_START_VAL` VARCHAR(50), IN `P_END_VAL` VARCHAR(50))  BEGIN 
	INSERT INTO OR_SET(OR_NUM_START, OR_NUM_END, DATE_ADDED)
	VALUES(P_START_VAL
			,P_END_VAL
			,CURRENT_TIMESTAMP);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_SELECT_ACCOUNTABLE_FORMS_REPORT` (IN `P_MONTH` INT, IN `P_WAY` VARCHAR(50))  BEGIN 
	IF P_WAY = 'MONTHLY' THEN
		...
		SELECT (SELECT (SELECT (OR_NUM_END + 1) - OR_NUM_START
							 FROM OR_SET
				  			 WHERE ID = (SELECT MAX(ID) FROM or_set)) +  (SELECT COUNT(*)
																					    FROM collection
																					    WHERE DATE(TRANS_DATE) >= (SELECT CONCAT(YEAR(NOW()),'-0',P_MONTH,'-',DAY(NOW()))))
				 )AS TOTAL_OR
				,(SELECT COUNT(*)
				  FROM COLLECTION
				  WHERE OR_NUM BETWEEN (SELECT OR_NUM_START FROM or_set WHERE ID = (SELECT MAX(ID) FROM or_set))
				  							AND
											  (SELECT OR_NUM_END FROM or_set WHERE ID = (SELECT MAX(ID) FROM or_set))
						AND MONTH(TRANS_DATE) = P_MONTH) AS TOTAL_USED
				,((OR_NUM_END + 1) - OR_NUM_START) - (SELECT COUNT(*)
															     FROM COLLECTION
															     WHERE OR_NUM BETWEEN (SELECT OR_NUM_START FROM or_set WHERE ID = (SELECT MAX(ID) FROM or_set))
															  	    						AND
																						     (SELECT OR_NUM_END FROM or_set WHERE ID = (SELECT MAX(ID) FROM or_set))
																		AND MONTH(TRANS_DATE) = P_MONTH) AS TOTAL_UNUSED
		FROM OR_SET
	...
	IF P_WAY = 'CARD' THEN
		SELECT (SELECT (SELECT (OR_NUM_END + 1) - OR_NUM_START
							 FROM OR_SET
				  			 WHERE ID = (SELECT MAX(ID) FROM or_set WHERE MONTH(DATE_ADDED) = P_MONTH)) +  (SELECT COUNT(*)
																																  	  FROM collection
...
('008870', '12000.00', '12000.00', NULL, 'NAWASA', 'Company', '2021-03-12 01:38:07', 'PENDING', 2),
('008871', '3500.00', '3500.00', NULL, 'DON, JUAN', 'NON-STUDENT', '2021-03-12 03:55:19', 'FOR REMIT', 2);
...
(116, 'PERMIT FEE', '12000.00', '008870'),
(117, 'CULTURAL FEES', '3500.00', '008871');
...
(253, '12346.00', '2021-DEPOSIT-00020', 'FOR REMIT', '2021-03-13 00:00:00', NULL, '2021-03-13 03:27:51', '', NULL, 2),
(254, '8871.00', '2021-DEPOSIT-00021', 'FOR REMIT', '2021-03-15 00:00:00', NULL, '2021-03-13 03:58:26', 'lunes', NULL, 2);
...
('j4olfvzo098u5x2ze7h06d5wtnoavfgp', 'N2RiNmI1ZTcyNmE5NDVjOTJjNzYxYzI3YzVlMzg1MDgzMDE1NWJlNzp7InVuYW1lIjoibWVncyIsInJvbGUiOjJ9', '2021-03-28 16:28:54.534440'),
...
INSERT INTO `or_set` (`ID`, `OR_NUM_START`, `OR_NUM_END`, `DATE_ADDED`) VALUES
(1, '008238', '008252', '2021-01-28 08:10:14'),
(2, '008630', '008642', '2021-02-03 08:10:33'),
(3, '008859', '008871', '2021-03-13 08:11:03');
