...
--
-- Host: 127.0.0.1
-- Generation Time: May 25, 2021 at 07:18 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERT_COLLECTION` (IN `P_ORNUM` VARCHAR(50), IN `P_ASSESSED` FLOAT, IN `P_COLLECTED` FLOAT, IN `P_STUDNO` VARCHAR(50), IN `P_CLIENT` VARCHAR(50), IN `P_EDUCLEVEL` VARCHAR(50), IN `P_USERID` VARCHAR(50))  BEGIN
	IF P_EDUCLEVEL = '' THEN
		IF P_STUDNO = '' THEN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERT_COLLECTION_BREAKDOWN` (IN `P_ORNUM` VARCHAR(50), IN `P_DESC` VARCHAR(50), IN `P_NATURE` VARCHAR(50), IN `P_AMT` FLOAT)  BEGIN 
	INSERT INTO collection_breakdown(`DESC`, NATURE, AMT, OR_NUM)
	VALUES(TRIM(P_DESC)
			,P_NATURE
			,P_AMT
			,P_ORNUM);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_INSERT_NEW_NATURE` (IN `P_UACS` VARCHAR(50), IN `P_NATURE` VARCHAR(50))  BEGIN
	INSERT INTO nature_of_collection(UACS_CODE, NATURE_NAME, DATE_ADDED)
	VALUES(P_UACS
			,UPPER(P_NATURE)
			,CURRENT_TIMESTAMP);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_LOAD_COLDEP_CARD` (IN `P_MONTH` INT)  BEGIN
	SELECT SUM(COLLECTED) AS COLLECTED
			,(SELECT SUM(COLLECTED)
			  FROM COLLECTION
			  INNER JOIN DEPOSIT
			  USING(`OR_NUM`)
			  WHERE MONTH(DATE_DEPOSITED) = P_MONTH) AS DEPOSITED
	FROM COLLECTION
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_REMOVE_NATURE` (IN `P_NAME` VARCHAR(50))  BEGIN 
	DELETE FROM NATURE_OF_COLLECTION
	WHERE NATURE_NAME = P_NAME;
END$$
	...
	IF P_WAY = 'CARD' THEN
		SELECT
			...
			,(SELECT IFNULL(LPAD(MAX(CAST(OR_NUM AS INT)),LENGTH(OR_NUM),'0'),LPAD('',(SELECT LENGTH(OR_NUM) 
			  ...
			  FROM collection 
			  WHERE MONTH(TRANS_DATE) = P_MONTH
			  		AND DATE(TRANS_DATE) <= (SELECT LAST_DAY(CURRENT_TIMESTAMP))) AS END_MONTH_OR
			,(SELECT COUNT(*)
			  FROM collection 
			  WHERE EDUC_LEVEL = 'SIS PAYMENT'
			  		AND MONTH(TRANS_DATE) = P_MONTH) AS SIS_COUNT
			,(SELECT COUNT(*)
			  FROM collection 
			  WHERE EDUC_LEVEL = 'COMPANY'
			  		AND MONTH(TRANS_DATE) = P_MONTH) AS COMPANY_COUNT
			,(SELECT COUNT(*)
			  FROM collection 
			  WHERE EDUC_LEVEL = 'NON-STUDENT'
			  		AND MONTH(TRANS_DATE) = P_MONTH) AS NON_STUDENT_COUNT
			,(SELECT COUNT(*)
			  FROM collection 
			  WHERE EDUC_LEVEL NOT IN('SIS PAYMENT','COMPANY','NON-STUDENT')
			  		AND MONTH(TRANS_DATE) = P_MONTH) AS ONSITE_COUNT;
	END IF;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_SELECT_CASH_RECEIPT_CARDS` (IN `P_MONTH` INT)  BEGIN 
	SELECT 
		 (SELECT COUNT(OR_NUM)
		  FROM collection
		  WHERE MONTH(TRANS_DATE) = P_MONTH) AS TOTAL_OR
		,(SELECT COUNT(SLIP_NUM)
		  FROM DEPOSIT
		  WHERE `STATUS` = 'DEPOSITED'
		  	AND MONTH(DATE_DEPOSITED) = P_MONTH) AS TOTAL_REF
		,(SELECT COUNT(DISTINCT CLIENT_NAME)
		  FROM COLLECTION
		  WHERE MONTH(TRANS_DATE) = P_MONTH) AS TOTAL_UNQ_PAYOR
		,(SELECT IFNULL(SUM(COLLECTED),0)
		  FROM COLLECTION
		  WHERE MONTH(TRANS_DATE) = P_MONTH) AS TOTAL_COLLECTION;	  
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_SELECT_COLLECTION_GROUP` (IN `P_MONTH` INT)  BEGIN
	SELECT COUNT(*)
			,`DESC`
			,SUM(AMT)
			,IF (C.OR_NUM IN (SELECT OR_NUM FROM deposit WHERE MONTH(DATE_DEPOSITED) = P_MONTH),'YES','NO')
	FROM collection_breakdown AS CB
	INNER JOIN collection AS C
	USING(`OR_NUM`)
	LEFT JOIN deposit AS D
	USING(`OR_NUM`)
	WHERE MONTH(C.TRANS_DATE) = P_MONTH
	GROUP BY `DESC`
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_SELECT_CURRENT_GROUP_ID` ()  BEGIN
	SELECT IF((SELECT COUNT(GROUP_ID) FROM deposit) = 0
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_SELECT_NATURE_COL` (IN `P_UACS` VARCHAR(50))  BEGIN
	SELECT NATURE_NAME
	FROM nature_of_collection
	WHERE UACS_CODE = P_UACS; 
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_SELECT_SUMCOL_CARDS` (IN `P_MONTH` INT)  BEGIN 
	SELECT 
		 (SELECT SUM(COLLECTED)
		 FROM collection
		 WHERE MONTH(TRANS_DATE) = P_MONTH) AS TOTAL_COLLECTION
		,(SELECT TOTAL_COLLECTION - SUM(AMT)
		 FROM collection_breakdown
		 INNER JOIN COLLECTION
		 USING(`OR_NUM`)
		 WHERE MONTH(TRANS_DATE) = P_MONTH) AS TOTAL_COLLECTION_NO_BREAKDOWN;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_SELECT_TOP_CONTRIB` (IN `P_MONTH` INT)  BEGIN 
	SELECT CLIENT_NAME AS `NAME`
			,COUNT(*) AS FREQUENCY
			,SUM(COLLECTED) AS AMOUNT
	FROM COLLECTION
	WHERE MONTH(TRANS_DATE) = P_MONTH
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_UPDATE_NATURE` (IN `P_OLD_NAME` VARCHAR(50), IN `P_NEW_NAME` VARCHAR(50))  BEGIN 
	UPDATE NATURE_OF_COLLECTION
	SET NATURE_NAME = UPPER(P_NEW_NAME)
	WHERE NATURE_NAME = P_OLD_NAME;
END$$
...
('00882510', '2500.00', '2500.00', '2017-00293-CM-0', 'SILLEZA, VAN ANTHONY', 'Bachelor\'s Degree', '2021-03-15 06:07:52', 'FOR REMIT', 2),
('00882511', '13288.00', '13288.00', NULL, 'PUP, QUEZON CITY', 'SIS Payment', '2021-03-15 09:33:04', 'FOR REMIT', 2),
('00882512', '1355.00', '1355.00', NULL, 'POTTER, NICOLAS', 'NON-STUDENT', '2021-03-18 06:57:15', 'FOR REMIT', 2),
('00882513', '4250.00', '4250.00', NULL, 'SINOVAC', 'Company', '2021-03-18 07:12:35', 'FOR REMIT', 2),
('00882514', '150.00', '150.00', '2018-00651-CM-1', 'PANLUBASAN, QUEEN KIMBERLY', 'Bachelor\'s Degree', '2021-03-18 07:23:17', 'FOR REMIT', 2),
('00882515', '24000.00', '24000.00', NULL, 'MERALCO', 'Company', '2021-03-18 08:57:58', 'FOR REMIT', 2),
('00882516', '3000.00', '3000.00', NULL, 'PUP, QUEZON CITY', 'SIS Payment', '2021-03-24 08:32:47', 'FOR REMIT', 2),
('00882517', '2000.00', '2000.00', '2017-00308-CM-0', 'ASILAN, MARK KENNEDY', 'Bachelor\'s Degree', '2021-03-24 09:32:25', 'FOR REMIT', 2),
('00882518', '28600.00', '28600.00', NULL, 'MERCURY DRUG', 'Company', '2021-03-24 09:34:22', 'FOR REMIT', 2),
('00882519', '2845.00', '2845.00', NULL, 'PUP, QUEZON CITY', 'SIS Payment', '2021-03-25 11:04:18', 'FOR REMIT', 2),
('00882520', '2020.00', '2020.00', NULL, 'PUP, QUEZON CITY', 'SIS Payment', '2021-03-27 03:11:27', 'FOR REMIT', 2),
('00882521', '1650.00', '1650.00', '2017-00389-CM-1', 'CANTUJA, MARINEL', 'Bachelor\'s Degree', '2021-03-31 11:05:32', 'FOR REMIT', 2),
('00882522', '150.00', '150.00', '2017-00293-CM-0', 'SILLEZA, VAN ANTHONY', 'Bachelor\'s Degree', '2021-03-31 12:14:39', 'FOR REMIT', 2),
('00882523', '175.00', '175.00', '2017-00273-CM-0', 'BRILLO, DAISY', 'Bachelor\'s Degree', '2021-03-31 12:23:30', 'FOR REMIT', 2),
('00882524', '150.00', '150.00', '2018-00651-CM-1', 'PANLUBASAN, QUEEN KIMBERLY', 'Bachelor\'s Degree', '2021-03-31 12:29:00', 'FOR REMIT', 2),
('00882525', '1850.00', '1850.00', NULL, 'TAO, PO', 'NON-STUDENT', '2021-04-02 08:28:36', 'FOR REMIT', 2),
('00882526', '1745.00', '1745.00', '2017-00293-CM-0', 'SILLEZA, VAN ANTHONY', 'Bachelor\'s Degree', '2021-04-14 12:10:19', 'FOR REMIT', 2),
('00882527', '750.00', '750.00', '2017-00389-CM-1', 'CANTUJA, MARINEL', 'Diploma', '2021-05-11 22:39:08', 'FOR REMIT', 2),
('00882528', '1964.00', '1964.00', NULL, 'PUP, QUEZON CITY', 'SIS Payment', '2021-05-12 23:51:11', 'FOR REMIT', 2),
('00882529', '1975.00', '1975.00', NULL, 'PUP, QUEZON CITY', 'SIS Payment', '2021-05-13 00:46:14', 'FOR REMIT', 2),
('00882530', '68001.00', '68001.00', NULL, 'NANDITO, AKO', 'NON-STUDENT', '2021-05-13 03:42:24', 'PENDING', 2),
('00882531', '125000.00', '125000.00', NULL, 'REX PRINTING COMPANY INC.', 'Company', '2021-05-13 03:43:29', 'FOR REMIT', 2),
('00882532', '13457.00', '13457.00', NULL, 'PUP, QUEZON CITY', 'SIS Payment', '2021-05-13 03:52:13', 'PENDING', 2),
('00882533', '1375.50', '1375.50', '2017-00306-CM-0', 'MERCURIO, JESSIE JAMES', 'Bachelor\'s Degree', '2021-05-18 04:06:32', 'PENDING', 2),
('00882534', '4792.36', '4792.36', NULL, 'PUP, QUEZON CITY', 'SIS Payment', '2021-05-18 04:07:18', 'PENDING', 2),
('00882535', '125000.00', '125000.00', NULL, 'SMART COMMUNICATIONS, INC.', 'Company', '2021-05-18 04:42:08', 'PENDING', 2);
...
INSERT INTO `collection_breakdown` (`ID`, `DESC`, `NATURE`, `AMT`, `OR_NUM`) VALUES
(121, 'GRADUATION FEE', NULL, '2500.00', '00882510'),
(122, 'PUBLICATION', NULL, '1355.00', '00882512'),
(123, 'LABORATORY FEE', NULL, '4250.00', '00882513'),
(124, 'CLEARANCE AND CERTIFICATION FEE', 'ICG-GRADES', '150.00', '00882514'),
(125, 'ELECTRICITY', NULL, '24000.00', '00882515'),
(126, 'DIPLOMA', NULL, '500.00', '00882517'),
(127, 'GRADUATION FEE', NULL, '1500.00', '00882517'),
(128, 'RENTAL', NULL, '28600.00', '00882518'),
(129, 'CULTURAL FEES', NULL, '2300.00', '00882519'),
(130, 'PUBLICATION', NULL, '200.00', '00882519'),
(131, 'ATHLETIC FEES', NULL, '345.00', '00882519'),
(132, 'BOOKS', NULL, '1270.00', '00882520'),
(133, 'MEDICAL, DENTAL', NULL, '500.00', '00882520'),
(134, 'LIBRARY FEES', NULL, '250.00', '00882520'),
(135, 'CLEARANCE AND CERTIFICATION FEE', 'ICG-GRADES', '150.00', '00882521'),
(136, 'TUITION FEES', NULL, '1500.00', '00882521'),
(137, 'CLEARANCE AND CERTIFICATION FEE', 'ICG-GRADES', '150.00', '00882522'),
(138, 'CLEARANCE AND CERTIFICATION FEE', 'CTC-GRADES', '175.00', '00882523'),
(139, 'CLEARANCE AND CERTIFICATION FEE', 'ICG-GRADES', '150.00', '00882524'),
(140, 'STATISTICAL CONSULTANCY', '', '1850.00', '00882525'),
(141, 'TRANSCRIPT OF RECORD FEES(SCANNED PHOTO)', '', '245.00', '00882526'),
(142, 'GRADUATION FEE', '', '1500.00', '00882526'),
(143, 'MEMORABILIA', '', '750.00', '00882527'),
(144, 'ATHLETIC FEES', '', '280.00', '00882528'),
(145, 'TUITION FEES', '', '1534.00', '00882528'),
(146, 'CLEARANCE AND CERTIFICATION FEE', 'ICG-GRADES', '150.00', '00882528'),
(149, 'TUITION FEES', '', '1745.00', '00882529'),
(150, 'ATHLETIC FEES', '', '230.00', '00882529'),
(151, 'ELECTRICITY', '', '68001.00', '00882530'),
(152, 'PUP SPONSOR SEMINAR', '', '125000.00', '00882531'),
(161, 'SCHOOL UNIFORM', '', '1375.50', '00882533'),
(162, 'PUP SPONSOR SEMINAR', '', '125000.00', '00882535');
...
(340, '00882511', '2021-DEPOSIT-00001', 'DEPOSITED', '2021-03-22 00:00:00', '2021-03-22 08:22:10', '2021-03-22 08:21:56', 'deposit for 3/22/2021', '2021-159', 2),
(341, '00882510', '2021-DEPOSIT-00001', 'DEPOSITED', '2021-03-22 00:00:00', '2021-03-22 08:22:10', '2021-03-22 08:21:56', 'deposit for 3/22/2021', '2021-159', 2),
(342, '00882512', '2021-DEPOSIT-00002', 'DEPOSITED', '2021-03-24 00:00:00', '2021-03-24 09:39:33', '2021-03-24 09:38:21', 'Collection from 03/24/2021', '2021-160', 2),
(343, '00882513', '2021-DEPOSIT-00002', 'DEPOSITED', '2021-03-24 00:00:00', '2021-03-24 09:39:33', '2021-03-24 09:38:21', 'Collection from 03/24/2021', '2021-160', 2),
(344, '00882514', '2021-DEPOSIT-00002', 'DEPOSITED', '2021-03-24 00:00:00', '2021-03-24 09:39:33', '2021-03-24 09:38:21', 'Collection from 03/24/2021', '2021-160', 2),
(345, '00882515', '2021-DEPOSIT-00002', 'DEPOSITED', '2021-03-24 00:00:00', '2021-03-24 09:39:33', '2021-03-24 09:38:21', 'Collection from 03/24/2021', '2021-160', 2),
(346, '00882521', '2021-DEPOSIT-00003', 'DEPOSITED', '2021-04-02 00:00:00', '2021-04-02 07:16:30', '2021-04-02 07:16:02', 'late deposit', '2021-161', 2),
(347, '00882522', '2021-DEPOSIT-00003', 'DEPOSITED', '2021-04-02 00:00:00', '2021-04-02 07:16:30', '2021-04-02 07:16:02', 'late deposit', '2021-161', 2),
(348, '00882523', '2021-DEPOSIT-00003', 'DEPOSITED', '2021-04-02 00:00:00', '2021-04-02 07:16:30', '2021-04-02 07:16:02', 'late deposit', '2021-161', 2),
(349, '00882520', '2021-DEPOSIT-00003', 'DEPOSITED', '2021-04-02 00:00:00', '2021-04-02 07:16:30', '2021-04-02 07:16:02', 'late deposit', '2021-161', 2),
(350, '00882524', '2021-DEPOSIT-00003', 'DEPOSITED', '2021-04-02 00:00:00', '2021-04-02 07:16:30', '2021-04-02 07:16:02', 'late deposit', '2021-161', 2),
(351, '00882519', '2021-DEPOSIT-00003', 'DEPOSITED', '2021-04-02 00:00:00', '2021-04-02 07:16:30', '2021-04-02 07:16:02', 'late deposit', '2021-161', 2),
(352, '00882516', '2021-DEPOSIT-00003', 'DEPOSITED', '2021-04-02 00:00:00', '2021-04-02 07:16:30', '2021-04-02 07:16:03', 'late deposit', '2021-161', 2),
(353, '00882517', '2021-DEPOSIT-00003', 'DEPOSITED', '2021-04-02 00:00:00', '2021-04-02 07:16:30', '2021-04-02 07:16:03', 'late deposit', '2021-161', 2),
(354, '00882518', '2021-DEPOSIT-00003', 'DEPOSITED', '2021-04-02 00:00:00', '2021-04-02 07:16:30', '2021-04-02 07:16:03', 'late deposit', '2021-161', 2),
(355, '00882526', '2021-DEPOSIT-00004', 'DEPOSITED', '2021-05-10 00:00:00', '2021-05-11 04:49:18', '2021-05-10 00:08:55', '', '2021-162', 2),
(356, '00882525', '2021-DEPOSIT-00004', 'DEPOSITED', '2021-05-10 00:00:00', '2021-05-11 04:49:18', '2021-05-10 00:08:55', '', '2021-162', 2),
(360, '00882529', '2021-DEPOSIT-00005', 'DEPOSITED', '2021-05-13 00:00:00', '2021-05-13 02:44:43', '2021-05-13 02:44:10', '', '2021-163', 2),
(361, '00882528', '2021-DEPOSIT-00005', 'DEPOSITED', '2021-05-13 00:00:00', '2021-05-13 02:44:43', '2021-05-13 02:44:10', '', '2021-163', 2),
(362, '00882527', '2021-DEPOSIT-00005', 'DEPOSITED', '2021-05-13 00:00:00', '2021-05-13 02:44:43', '2021-05-13 02:44:10', '', '2021-163', 2),
(363, '00882531', '2021-DEPOSIT-00006', 'DEPOSITED', '2021-05-16 00:00:00', '2021-05-16 04:47:36', '2021-05-16 04:46:59', '', '2021-164', 2);
...
('86njqt7c7j9rskgwodfu73epukhy0bvh', 'N2RiNmI1ZTcyNmE5NDVjOTJjNzYxYzI3YzVlMzg1MDgzMDE1NWJlNzp7InVuYW1lIjoibWVncyIsInJvbGUiOjJ9', '2021-06-03 19:16:08.352065'),
...
('j4olfvzo098u5x2ze7h06d5wtnoavfgp', 'N2RiNmI1ZTcyNmE5NDVjOTJjNzYxYzI3YzVlMzg1MDgzMDE1NWJlNzp7InVuYW1lIjoibWVncyIsInJvbGUiOjJ9', '2021-05-01 07:34:39.485568'),
...
--
-- Table structure for table `nature_of_collection`
--

CREATE TABLE `nature_of_collection` (
  `ID` int(11) NOT NULL,
  `UACS_CODE` varchar(100) DEFAULT NULL,
  `NATURE_NAME` varchar(50) DEFAULT NULL,
  `DATE_ADDED` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

--
-- Dumping data for table `nature_of_collection`
--

INSERT INTO `nature_of_collection` (`ID`, `UACS_CODE`, `NATURE_NAME`, `DATE_ADDED`) VALUES
(1, '4 02 01 040 00 00000 ', 'ICG-GRADES', '2021-03-29 12:18:14'),
(2, '4 02 01 040 00 00000 ', 'CTC-GRADES', '2021-03-29 12:18:37');
...
(4, '00882510', '00882610', '2021-03-15 05:51:48', 101, 58);
...
--
-- Indexes for table `nature_of_collection`
--
ALTER TABLE `nature_of_collection`
  ADD PRIMARY KEY (`ID`),
  ADD UNIQUE KEY `NATURE_NAME` (`NATURE_NAME`),
  ADD KEY `UACS_CODE` (`UACS_CODE`);
...
--
ALTER TABLE `deposit`
  MODIFY `DEP_ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=368;
...
--
-- AUTO_INCREMENT for table `nature_of_collection`
--
ALTER TABLE `nature_of_collection`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=7;
...
--
-- Constraints for table `nature_of_collection`
--
ALTER TABLE `nature_of_collection`
  ADD CONSTRAINT `nature_of_collection_ibfk_1` FOREIGN KEY (`UACS_CODE`) REFERENCES `uacs` (`UACS_CODE`);
